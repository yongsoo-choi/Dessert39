...
class StoreInfoPages: UIPageViewController {
...
extension StoreInfoPages: UIPageViewControllerDelegate, UIPageViewControllerDataSource {
    ...
    func pageViewController(_ pageViewController: UIPageViewController, viewControllerBefore viewController: UIViewController) -> UIViewController? {
        ...
        if previousIndex < 0 {
            store.storeInfoIndex = VCArray.count - 1
            return VCArray.last
        } else {
            store.storeInfoIndex = previousIndex
    ...
    func pageViewController(_ pageViewController: UIPageViewController, viewControllerAfter viewController: UIViewController) -> UIViewController? {
        ...
        if nextIndex >= VCArray.count {
            store.storeInfoIndex = 0
            return VCArray.first
        } else {
            store.storeInfoIndex = nextIndex
